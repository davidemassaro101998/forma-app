...
import { Flame, CheckCircle, Dumbbell, Check } from "lucide-react";
...
export const LoadingApple3D: React.FC<LoadingApple3DProps> = React.memo(({ language = "en", subtitle }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center text-center p-4 sm:p-6 select-none bg-[#F2F2F7] relative overflow-y-auto">
      {/* Background Soft Green Ambient Depth */}
      <div className="absolute w-80 h-80 bg-[#0EA968]/10 rounded-full blur-3xl -top-16 -left-16 pointer-events-none" />
      <div className="absolute w-80 h-80 bg-[#000000]/5 rounded-full blur-3xl -bottom-16 -right-16 pointer-events-none" />

      {/* Cinematic 3D Fitness Assembly Stage (fluidly scaled to viewport height so it never gets clipped in short/narrow previews) */}
      <div className="relative mb-[clamp(0.75rem,4vh,2rem)] w-[clamp(6rem,26vh,11rem)] h-[clamp(6rem,26vh,11rem)] shrink-0 flex items-center justify-center">
        ...
        <motion.div
          ...
          className="absolute inset-0 rounded-full border-2 border-dashed border-[#0EA968]/40"
        />
        ...
        {/* Floating Fitness Assembly Vector Animation */}
        <div className="relative z-10 w-[64%] h-[64%] flex items-center justify-center">
          <motion.div
            ...
            className="w-[72%] h-[72%] rounded-2xl bg-white border border-[#0EA968] shadow-xl flex items-center justify-center relative overflow-hidden"
          >
            {/* Shimmer light pass */}
            <motion.div
              animate={{ x: ["-100%", "200%"] }}
              transition={{ duration: 2, repeat: Infinity, ease: "easeInOut", repeatDelay: 1 }}
              className="absolute inset-0 bg-gradient-to-r from-transparent via-[#0EA968]/10 to-transparent -skew-x-12"
            />

            <Dumbbell className="w-[45%] h-[45%] text-[#0EA968]" />
          </motion.div>
          ...
          <motion.div
            ...
            className="absolute top-1 right-1 text-[#0EA968]"
          >
            <Flame className="w-[clamp(0.9rem,3.5vh,1.5rem)] h-[clamp(0.9rem,3.5vh,1.5rem)] fill-[#0EA968]" />
          </motion.div>
          ...
          return (
            <div
              ...
              className={`p-3 rounded-[16px] border text-xs font-semibold flex items-center gap-3 transition-all duration-300 ${
                isCurrent
                  ? "bg-white border-[#0EA968] text-[#000000] shadow-[0_4px_12px_rgba(0,0,0,0.06)] font-extrabold"
                  : isDone
              ...
              <div
                className={`w-5 h-5 rounded-full flex items-center justify-center shrink-0 text-[10px] ${
                  isDone
                    ...
                    : isCurrent
                    ? "bg-[#0EA968] text-white font-extrabold"
