      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            <div className="w-16 h-16 rounded-full bg-[#0EA968]/10 flex items-center justify-center shrink-0">
              <Mic className="w-7 h-7 text-[#0EA968] stroke-[2.2]" />
            </div>

            <div className="text-center space-y-1.5">
              <h2 className="text-lg font-bold text-[#000000]">
                {language === "it" ? "Benvenuto su Forma AI" : "Welcome to Forma AI"}
              </h2>
              <p className="text-sm text-[#000000] leading-relaxed font-medium">
                {language === "it"
                  ? "L'AI ti trova il prodotto fitness giusto su Amazon in 3 tap: dì o scegli dove ti alleni, l'obiettivo e il budget."
                  : "AI finds the right fitness product on Amazon in 3 taps: say or pick where you train, your goal, and your budget."}
              </p>
              <p className="text-sm text-[#8E8E93] leading-relaxed">
                {language === "it"
                  ? "Tocca il microfono e descrivi a voce il tuo obiettivo: se al primo utilizzo il telefono chiede il permesso al microfono, tocca \"Consenti\" per usare la ricerca vocale."
                  : "Tap the mic and describe your goal out loud: if your phone asks for microphone permission the first time, tap \"Allow\" to use voice search."}
              </p>
            </div>
            ...
            {showInstallTip && (
              <div className="w-full rounded-[18px] bg-[#F2F2F7] p-3.5 flex items-start gap-3">
                <div className="w-8 h-8 rounded-xl bg-white shadow-sm flex items-center justify-center shrink-0 mt-0.5">
                  {platform === "ios" ? (
                    <Share className="w-4 h-4 text-[#0EA968]" />
                  ) : (
                    <MoreVertical className="w-4 h-4 text-[#0EA968]" />
                  )}
                </div>
            ...
            <button
              type="button"
              onClick={onDismiss}
              className="w-full py-3.5 px-6 rounded-[20px] bg-[#0EA968] hover:bg-[#0B8452] text-white font-black text-sm uppercase tracking-wider active:scale-[0.98] transition-all cursor-pointer mt-1"
            >
