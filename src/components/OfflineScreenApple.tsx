  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="fixed inset-0 z-[99999] flex flex-col items-center justify-center p-6 text-center select-none font-sans"
      ...
      >
        ...
        >
          ...
          <button
            onClick={handleRetry}
            disabled={isChecking}
            className="w-full mt-2 py-3.5 px-4 rounded-[18px] bg-[#0EA968] text-white font-bold text-xs uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer shadow-md active:scale-[0.97] transition-all border border-[#0EA968] disabled:opacity-60"
          >
            <RefreshCw className={`w-4 h-4 ${isChecking ? "animate-spin" : ""}`} />
            <span>
              {isChecking ? t.offlineCheckingBtn : t.offlineRetryBtn}
            </span>
          </button>
        </motion.div>
      </motion.div>
