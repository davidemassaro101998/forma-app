...
describe("generateSmartFallbackGifts", () => {
  ...
  it("always returns exactly 3 products with prices inside the requested budget", () => {
  ...
  it("generates non-Italian copy when a non-Italian language is requested", () => {
    ...
    // Italian product titles no matter what. This must not happen again.
    const gifts = generateSmartFallbackGifts(quiz({ budget: ">100€" }), IT_COUNTRY, "de");
    for (const gift of gifts) {
      expect(gift.title.toLowerCase()).not.toContain("bilanciere");
    }
  });
  ...
  it("matches the yoga & mobility topic override regardless of language", () => {
    for (const language of languages) {
      const gifts = generateSmartFallbackGifts(quiz({ extraDetails: "yoga per principianti" }), IT_COUNTRY, language);
      expect(gifts.some((g) => g.category === "yoga")).toBe(true);
    }
  });

  it("matches the strength/gym topic override regardless of language", () => {
    for (const language of languages) {
      const gifts = generateSmartFallbackGifts(quiz({ extraDetails: "voglio allenare la forza in palestra" }), IT_COUNTRY, language);
      expect(gifts.some((g) => g.category === "strength")).toBe(true);
    }
  });

  it("always tags exactly one card per diversification slot (top pick / essential / top quality)", () => {
